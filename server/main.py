...
def env():
    ...
    return {
        ...
        #    自带那份，所以正常情况下它恒为真。现在的实际含义是
        #    「XSL 找得到吗」，`path` 还能看出用的是自带那份还是用户 Office 的。
        #
        #    ⚠️ **前端已经没有任何地方读它了**（同日把拦截屏那个
        #    `(e.office||{}).ok` 判据去掉之后就没消费方了）。留着是因为
        #    `path` 在排错时有用 —— 一眼看出 XSL 是从哪来的。
        #    真要删的话，前端 goodEnv 那份假数据也得一起改；
        #    在那之前别把它当成「有人在用」的字段。
