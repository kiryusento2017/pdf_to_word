...
NOT_PROJECT_FILES = {
    'PDF转Word-v0.0.1.exe', 'PDF.Word-v0.0.1.exe',   # 早期改过名的产物
    ...
    'requires.json', 'requires-vX.json',  # 文档里的占位写法
...
def is_release_asset(name):
    r"""是不是发行版产物（Release 的三个附件）。

    这些东西在 `dist/` 里，不在源码树 —— 文档提到它们是正常的。
    **按模式认，不要往白名单里一版一版加**：原来那儿硬编码着
    `PDF2Word-Setup-v0.1.0.exe`，于是 README 一升版本号这条检查就报
    「缺文件」，而真正该被发现的问题反倒被这声噪音盖住。
    """
    return ((name.startswith('PDF2Word-Setup-v') and name.endswith('.exe'))
...
def truth():
    ...
    # 🔴 取**最新打出来**的那个安装包，不是写死某一版。
    #    原来这儿是 `dist/PDF2Word-Setup-v0.0.1.exe` —— 于是 README 里的
    #    「287 MB」永远在跟第一版比。碰巧四舍五入相同才一直没暴露，
    #    真到包体积变了那天，这条检查会安静地放行一个错数字。
    import glob
    cands = sorted(glob.glob('dist/PDF2Word-Setup-v*.exe'),
                   key=os.path.getmtime)
    t['exe_mb'] = (round(os.path.getsize(cands[-1]) / 1048576.0)
                   if cands else 0)
...
def check_numbers(t):
    ...
    🔴 **测试条数这一项跳过进度档。** 进度档是流水账，每次发版都会留下
       「当时是多少条」的记录（v0.1.0 那次 445、v0.2.6 那次也 445），
       而这里是拿每个数去跟**当前真值**比 —— 于是历史记录必然被判成
       「数不对」，而唯一能让它闭嘴的办法是把发版留档改成今天的数，
       那等于篡改历史。

       2026-09-06 撞上：进度档里五处 445 全是历史，只有一处 446 是当前值，
       门禁却报「写 445 实际 446」。**这是判据的毛病，不是文档写错了。**

       README / DESIGN / RELEASE 仍然严格查 —— 那三份描述的是当前状态，
       里面出现旧数字就是真的过时了。进度档不在项目里、不对外、天然带历史，
       数字对不上是常态。

       跳的是**这一整节的四项**（Python 条数、前端条数、命令注释、安装包
       MB），不是只跳条数 —— 四项都会在发版留档里留下当时的值，
       只挡一项等于留着另外三个同样的坑。
    """
    for doc in DOCS:
        if not os.path.isfile(doc):
