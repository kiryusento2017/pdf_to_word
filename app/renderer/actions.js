  ...
  window.P2W_ACTS = {
    ...
    doRollback: function (name) {
      ...
      HTTP.post('/api/upgrade/rollback', { name: name }).then(function (d) {
        ...
        if (d && d.ok) {
          // 🔴 以前这里只弹一句「点『立即重启』」，**而那个按钮根本不
          //    存在** —— 文案在指挥用户去点一个没有的东西。现在置这个
          //    标记，备份列表那块换成真的「立即重启」按钮。
          //
          //    重启到底为什么必要：服务进程自己**不 import torch**
          //    （torchdep 读的是 site-packages/torch/version.py 这个
          //    文件），转换又是每份新起一个子进程 —— 所以功能上不重启
          //    也已经是退回后的版本了。要重启是因为**界面上那些版本、
          //    占用、备份列表都是开机时读进来的**，不重启一直显示旧数。
