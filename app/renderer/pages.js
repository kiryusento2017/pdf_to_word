...
// 🔴 **转换中不再禁用「关于」**（小蔡 2026-09-08 改）。
//
//    2026-09-03 禁用它的理由是：进去第一个按钮就是「检查更新」，而更新包
//    覆盖的正是 `pipeline/*.py` —— 转到一半换掉代码，后面几份读到的是新
//    代码；装完还要重启，一重启这批全废。
//
//    但那是**为了拦一个按钮，把整间屋子锁了**。屋里其余的东西（版本号、
//    环境检测、磁盘占用、**生成诊断文件**）转换中看全都是安全的，而诊断
//    恰恰是转换卡住时最需要的那一个 —— 以前正好进不去。
//
//    所以保护点下移：门开着，**「检查更新」自己变灰**（见 aboutView）。
//    「更新模型」「清理」本来就各自灰着，后端那四道互斥也一行没动。
function botBar(st, extra, compact) {
  return envLine(st, compact)
    // 🔴 「历史」排在「关于」前面 —— 常用的靠左。两个转换中都能点：
    //    历史纯只读，关于里面的危险动作各自灰着。
    + btn('openHistory', '历史', { cls: 'link' })
    + btn('openAbout', '关于', { cls: 'link' })
...
function aboutView(st) {
  ...
  return '<div class="fill" style="justify-content:flex-start;'
    ...
    //
    //    以前这个按钮光秃秃没有任何保护，全靠外面底栏的「关于」变灰挡着。
    //    现在那道门开了（转换中也能进来看诊断），它就直接暴露在外，
    //    所以保护得挪到自己身上。
    //
    //    为什么它比屋里其他按钮都危险：更新包覆盖的正是 `pipeline/*.py`，
    //    而转换每处理一份 PDF 就新起一次 MinerU 子进程 —— 转到一半换掉
    //    代码，后面几份读到的是新代码；装完还要重启，一重启这批全废，
    //    而老师那边可能已经等了十几分钟。
    + btn('checkUpdate', '检查更新',
          { cls: 'primary', off: isRunning(st),
...
function envCheckView(st) {
  ...
  return '<div class="fill" style="justify-content:flex-start;'
    ...
    // 🔴 **转换中不再禁用**（2026-09-08 改）。它只是上网问一句
    //    「torch 和 mineru 有没有新版本」，一个字节都不写，跟正在跑的
    //    转换没有任何交集。以前跟着「关于」一起被锁在门外，属于误伤。
    //    （旁边的「更新模型」和「清理」保持禁用 —— 那两个是真会动文件的。）
