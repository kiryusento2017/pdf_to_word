...
下载 `PDF2Word-Setup-v0.2.1.exe`（287 MB），双击 → 选文件夹 → 解压完成 →
双击里面的 `PDF转Word.exe`。
...
tests/      431 条 Python + 139 条前端检查 + 四个真实数据验证脚本
...
```
.venv\Scripts\python.exe -m unittest discover -s tests -q   # 431 条，16 秒
