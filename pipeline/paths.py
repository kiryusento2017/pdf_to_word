...
def child_env(source_env=None):
    ...
    2026-09-06 踩到过一次：一份 56 页讲义在离终点 18 分钟处被掐。
    ⚠️ **当时那个速率样本是脏的** —— 机器上还有另一个软件在抢 GPU。
    同一份 PDF 在干净环境下 1814 秒（30 分 14 秒）就跑完了，32.4 秒/页，
    613 个公式转成 612 个原生公式。出厂估值 SEC_PER_PAGE_GPU = 26.0 其实
    相当准（偏低约 25%），并不是差好几倍。

    所以这个上限**不是给「页数多」兜底的，是给「有东西抢 GPU」兜底的**：
    干净环境按 32.4 秒/页折算，1 小时能跑约 110 页；而有东西抢显卡时，
    同一份 56 页的连 1 小时都跑不完。
    🔴 别再拿页数推「多少页必挂」—— 这段注释的第一版就是拿那一个脏样本
    推出「约 54 页是红线、确定性不是偶发」，被小蔡一句「上次是两个软件
    一起工作」直接推翻。**一个样本推不出确定性结论。**

    一份 PDF 是一个 task、各算各的（server 那边逐份串行转），所以这里管的
    是「单份最多等多久」。24 小时按上面的速率够跑约 2600 页；真跑满一天
    几乎必然是卡死而非在算，留这个上限是留一个最终结论（用户随时能按停止，
    不必靠它）。
