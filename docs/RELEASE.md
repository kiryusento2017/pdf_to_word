...
```
.venv\Scripts\python.exe -m unittest discover -s tests -q   # 626 条
.venv\Scripts\python.exe tools\check_upstream.py            # 上游有没有新版
node tests\front_check.js                                   # 219 条
```
...
```
PDF2Word-Setup-vX.exe       291 MB    新用户下这个
...
| 依赖不满足（加了新的 pip 包） | 去下载页，手动覆盖安装 | 291 MB |
| 手上是 v0.0.1 / v0.0.2 | **只能手动**（那两版更新功能是坏的） | 291 MB |

**跨多少个版本都是一步到位，不用逐个爬。** 更新包是全量替换；
完整安装包更是本来就完整。区别只在下 0.5 MB 还是 291 MB。
...
| 挑附件 | 挑成完整包，为 0.5 MB 的改动下 291 MB | `_pick_asset` 只认名字带 update 的 zip |
...
小蔡 2026-09-09 在 GitHub 上看出来的：「为什么好多发行版的文案都挤在左边，
明明可以塞满一整行」。

**原因**：GitHub 的 Release 说明走 GFM，**单个换行会渲染成真正的断行**
（`<br>`），不像标准 Markdown 那样把软换行当空格合并。所以源码里折到
70 字符，网页上就在那儿断，右边空一大片 —— 一段话被切成好几条短行。
...
好（一段一行，让 GitHub 自己排）：
    「关于 → 环境检测」多了个「生成诊断文件」。点一下，软件把能摸到的一切写成一个 txt，然后弹出资源管理器并选中它，直接拖进微信发出来就行。
```

**只有正文段落中招。** 摘要那几条 `- 新增…` 是列表项，本来就会自动重排；
标题、表格、代码块同理，都不用管 —— 代码块里的换行**必须**原样保留。

⚠️ 这条跟本项目其它文档的写法相反：`.md` 文件在编辑器里读，折行到 70~80
字符是对的；**只有发布说明例外**，因为它的渲染环境把换行当断行。写的时候
别顺手套用平时的习惯。

2026-09-09 一次性把 v0.0.2 ~ v0.3.1 十三个历史版本的说明都合并过了
（v0.0.1 和当时刚发的 v0.3.1 本来就没问题）。
